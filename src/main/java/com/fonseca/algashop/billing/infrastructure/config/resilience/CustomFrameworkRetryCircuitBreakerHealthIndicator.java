...
@Component("circuitbreakres")
public class CustomFrameworkRetryCircuitBreakerHealthIndicator implements HealthIndicator {

    List<FrameworkRetryCircuitBreaker> circuitBreakers = new ArrayList<>();

    public CustomFrameworkRetryCircuitBreakerHealthIndicator(CircuitBreakerFactory circuitBreakerFactory) {
        circuitBreakers.add((FrameworkRetryCircuitBreaker) circuitBreakerFactory.create(fastpayCBWithRetryId));
        circuitBreakers.add((FrameworkRetryCircuitBreaker) circuitBreakerFactory.create(fastpayCBNoWithRetryId));
    }
